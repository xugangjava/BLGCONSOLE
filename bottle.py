...
views engines - all in a single file and with no dependencies other than the
    ...
    def apply(self, callback, route):
        ...
        def wrapper(*a, **ka):
            ...
            if isinstance(rv, dict):
                ...
                json_response = dumps(rv,cls=CJsonEncoder)
...
class TemplatePlugin(object):
    ''' This plugin applies the :func:`view` decorator to all routes with a
        `views` config parameter. If the parameter is a tuple, the second
        element must be a dict with additional options (e.g. `template_engine`)
        or default variables for the views. '''
    name = 'views'
    api = 2

    def apply(self, callback, route):
        conf = route.config.get('views')
...
class BaseTemplate(object):
    """ Base class and minimal API for views adapters """
    ...
    def __init__(self, source=None, name=None, lookup=[], encoding='utf8', **settings):
        ...
        is used to guess a views filename. Subclasses can assume that
        ...
        if not self.source and not self.filename:
            raise TemplateError('No views specified.')
    ...
    def search(cls, name, lookup=[]):
        ...
        if not lookup:
            depr('The views lookup path list should not be empty.')  # 0.12
            lookup = ['.']

        if os.path.isabs(name) and os.path.isfile(name):
            depr('Absolute views path names are deprecated.')  # 0.12
    ...
    def prepare(self, **options):
        ...
        It should be possible to call this again to refresh a views or to
        update settings.
        """
        raise NotImplementedError

    def render(self, *args, **kwargs):
        """ Render the views with the specified local variables and return
...
class SimpleTemplate(BaseTemplate):
    ...
    def render(self, *args, **kwargs):
        """ Render the views using keyword arguments as local variables. """
        env = {};
        stdout = []
...
class StplParser(object):
    ...
    # 8: A customizable end-of-code-block views token (only end of line)
    _re_tok += '|(%(block_close)s[ \\t]*(?=\\r?$))'
    # 9: And finally, a single newline. The 10th token is 'everything else'
    _re_tok += '|(\\r?\\n)'

    # Match the start tokens of code areas in a views
    ...
    def read_code(self, multiline):
        ...
        while True:
            ...
            if _str:  # Python string
                ...
                self.indent -= 1
            elif _cend:  # The end-code-block views token (usually '%>')
                if multiline:
                    multiline = False
                else:
                    code_line += _cend
...
def template(*args, **kwargs):
    '''
    Get a rendered views as a string iterator.
    You can use a name, a filename or a views string as first parameter.
...
def view(tpl_name, **defaults):
    ''' Decorator: renders a views for a handler.
        The handler can control its behavior like that:

          - return a dict of views vars to fill out the views
          - return something other than a dict and the view decorator will not
            process the views, but return the handler result as is.
...
#: The default views used for error pages. Override with @error()
ERROR_PAGE_TEMPLATE = """
%%try:
